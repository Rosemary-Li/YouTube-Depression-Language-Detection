...
The analysis unit is the **comment**, not the video. A music video has no depression relevance — but a comment underneath it saying *"I've been struggling so much lately, this is the only thing keeping me going"* does. We are measuring the signal at the comment level, across video contexts.
...
│   ├── video_analysis.py     # Step 1: Video-level topic classification
│   ├── comment_analysis.py   # Step 2: Three-layer depression signal detection
│   ├── linking.py            # Step 3: Cross-category comparison and statistical tests
...
| 3 — Cross-category linking | `linking.py` | both feature CSVs | `data/outputs/heatmap.png`, `data/outputs/umap.png` |
...
# Step 3: cross-category comparison
...
## Depression Signal Detection (Three Layers)
...
**Layer 1 — Sentiment score** (broad signal)
Every comment is scored positive / negative / neutral using `cardiffnlp/twitter-roberta-base-sentiment`. This is a baseline feature, not a depression indicator on its own.

**Layer 2 — Depression lexicon matching** (domain-specific)
Comments are matched against a mental health lexicon (LIWC categories: negative emotion, sadness, anxiety, health) and a depression-specific wordlist covering terms such as "hopeless", "worthless", "exhausted", "numb", "empty". This filters for domain-relevant language beyond general negativity.

**Layer 3 — Self-disclosure pattern detection** (high-precision)
First-person pronoun + depression-indicative vocabulary combinations are identified (e.g. "I feel hopeless", "I've been struggling", "I can't do this anymore"). These comments represent genuine personal disclosure rather than reactions to video content, and carry the strongest research signal.
...
| Video topic classification | `facebook/bart-large-mnli` (zero-shot) |
| Depression lexicon | LIWC (negative emotion, sadness, anxiety, health) |
